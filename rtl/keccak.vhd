...
--capacity part for rate 576 (SHA3-512)
...
--rate part for rate 576 (SHA3-512)
	i11: for col in 0 to 4 generate
		i12: for i in 0 to 63 generate
			round_in_rate576(0)(col)(i)<= reg_data(0)(col)(i) xor (din_buffer_out((0*64*5)+(col*64)+i) and (din_buffer_full_temp and permutation_computed));
		end generate;	
	end generate;

	i13: for col in 0 to 3 generate
		i14: for i in 0 to 63 generate
			round_in_rate576(1)(col)(i)<= reg_data(1)(col)(i) xor (din_buffer_out((1*64*5)+(col*64)+i) and (din_buffer_full_temp and permutation_computed));
		end generate;	
	end generate;


--capacity part for rate 1088 (SHA3-256 and SHAKE256)
...
--rate part for rate 1088 (SHA3-256 and SHAKE256)
	i10rate1088: for row in 0 to 2 generate
		i11rate1088: for col in 0 to 4 generate
			i12rate1088: for i in 0 to 63 generate
				round_in_rate1088(row)(col)(i)<= reg_data(row)(col)(i) xor (din_buffer_out((row*64*5)+(col*64)+i) and (din_buffer_full_temp and permutation_computed));
			end generate;	
		end generate;
	end generate;

	i13rate1088: for i in 0 to 63 generate
			round_in_rate1088(3)(0)(i)<= reg_data(3)(0)(i) xor (din_buffer_out((3*64*5)+(0*64)+i) and (din_buffer_full_temp and permutation_computed));
			round_in_rate1088(3)(1)(i)<= reg_data(3)(1)(i) xor (din_buffer_out((3*64*5)+(1*64)+i) and (din_buffer_full_temp and permutation_computed));
	end generate;	



--capacity part for rate 1344 (SHAKE128)
...
--rate part for rate 1344 (SHAKE128)
...
-- *** CRITICAL FIX: Separate SHAKE256 from SHAKE128 ***
-- SHAKE256 uses 17-word rate (like SHA3-256), NOT 21-word rate (like SHAKE128)
round_in <= round_in_rate576  when (rate_type = "01") else -- SHA3-512 (9 words)
            round_in_rate1088 when (rate_type = "00") else -- SHA3-256 (17 words)
            round_in_rate1344 when (rate_type = "10") else -- SHAKE128 (21 words)
            round_in_rate1088 when (rate_type = "11") else -- SHAKE256 (17 words) *** FIXED ***
            round_in_rate1088; -- safe default (17 words)
