...
module TB_KG_SL5;
    ...
    initial begin
        ...
        @(posedge clk);
        ins = {`RTYPE'd3, `IV'd0, `WE'd0, `ADDR'd0, `ADDR'd0, `ADDR'd0, `OP'd2, `CDT'd0, `TID'd2}; // operating shake-128 in parallel with sampling rejection (write from addr 4 to 925)
        @(posedge clk); 
        wait (TOD == `TOD'd1);
        ...
        for(j=0; j<105; j=j+1) begin
        
            iv0 = 2*j;
            iv1 = (2*j) + 1'b1;
            addr_init_Pi3 = 3570 + (j*1225); //(1225 is calculated using M*M = 35*35)
            ...
            // Generating matrix Pi1 (see address 3570 to 8822 of memory )
            ins = {`RTYPE'd3, iv0, `WE'd0, `ADDR'd3570, `ADDR'd0, `ADDR'd4, `OP'd3, `CDT'd0, `TID'd2}; // before here, `IV'd0 was
            @(posedge clk); 
            wait (TOD == `TOD'd1);
            ...
            // Generating matrix Pi2 (see address 8823 to 12392 of memory )
            ins = {`RTYPE'd3, iv1, `WE'd0, `ADDR'd8823, `ADDR'd0, `ADDR'd4, `OP'd4, `CDT'd0, `TID'd0}; // before here, `IV'd1 was
            @(posedge clk); 
            wait (TOD == `TOD'd1);
            ...
            // Executing MATRIX_MxV_MUL_SYMMETRIC_MATRIX_VxV (SdT,Pi1,TMP); (see addresses from 12393 to 15962 )
            ins = {`RTYPE'd3, `IV'd0, `WE'd0, `ADDR'd12393, `ADDR'd3570, `ADDR'd0, `OP'd5, `CDT'd0, `TID'd2}; 
            @(posedge clk); 
            wait (TOD == `TOD'd1);
            ...
            // Executing MATRIX_SUB_MxV(Pi2T,TMP,TMP) ; (write in mem2 from addresses 0 to 3569 using V*M )
            ins = {`RTYPE'd3, `IV'd0, `WE'd0, `ADDR'd0, `ADDR'd12393, `ADDR'd8823, `OP'd6, `CDT'd0, `TID'd2}; 
            @(posedge clk); 
            wait (TOD == `TOD'd1);
            ...
            // Executing MATRIX_MUL_MxV_VxM(TMP,Sd,P3[i]) ; (write from 15963 to 17187 )
            ins = {`RTYPE'd3, `IV'd0, `WE'd0, `ADDR'd15963, `ADDR'd0, `ADDR'd0, `OP'd7, `CDT'd0, `TID'd2}; 
            @(posedge clk); 
            wait (TOD == `TOD'd1);
            ...
            // Executing MATRIX_MUL_ADD_MxV_VxM(SdT,Pi2,P3[i]) ; (write from 17188 to xxxx)
            ins = {`RTYPE'd3, `IV'd0, `WE'd0, `ADDR'd17188, `ADDR'd8823, `ADDR'd0, `OP'd8, `CDT'd0, `TID'd2}; 
            @(posedge clk); 
            wait (TOD == `TOD'd1);
            ...
            // Executing last addition (write in mem2 addresses from 3570 to 132,194)
            ins = {`RTYPE'd3, `IV'd0, `WE'd0, addr_init_Pi3, `ADDR'd15963, `ADDR'd17188, `OP'd9, `CDT'd0, `TID'd2}; 
            @(posedge clk); 
            wait (TOD == `TOD'd1);
